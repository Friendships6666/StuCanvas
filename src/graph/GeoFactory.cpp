...
namespace {
     ...
     */
    void CompileAndLinkRPNInternal(GeometryGraph &graph, uint32_t id, const std::string &infix_expr, std::vector<uint32_t>& out_parents) {
        auto &node = graph.get_node_by_id(id);
    ...
    void Render_Point_Delegate(GeoNode& self, const std::vector<GeoNode>& pool, const std::vector<int32_t>& id_map,
                               const ViewState& v, const NDCMap& m, oneapi::tbb::concurrent_bounded_queue<FunctionResult>& q) {
