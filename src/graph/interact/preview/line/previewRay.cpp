...
uint32_t EndRay_Interact(GeometryGraph& graph) {
    ...
    GeoFactory::CreateRay(graph,graph.preview_registers[0],graph.preview_registers[1],graph.preview_visual_config);
...
uint32_t InitRay_Interact(GeometryGraph& graph) {
    ...
    if (graph.is_alive(selected_id)) {
        ...
        if (GeoType::is_point(selected_node.type)) {
            ...
            graph.preview_func = PreviewRay_Intertact;
            graph.preview_type = GeoType::LINE_RAY;
            graph.preview_registers[0] = selected_id;
            graph.next_interact_func = EndRay_Interact;
