...
static WGPUShaderModule ImGui_ImplWGPU_CreateShaderModule(const char* wgsl_source)
{
    ...
    wgsl_desc.code = { wgsl_source, WGPU_STRLEN }; // 使用 WGPUStringView 包装
...
void ImGui_ImplWGPU_UpdateTexture(ImTextureData* tex)
{
    ...
    if (tex->Status == ImTextureStatus_WantCreate)
    {
        ...
        auto* backend_tex = IM_NEW(ImGui_ImplWGPU_Texture)();

        // Create texture
        WGPUTextureDescriptor tex_desc = {};
        // 💡 修复 1：统一使用 WGPUStringView (Dawn 标准)
    ...
    if (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        // 💡 修复 2：统一使用 WGPUTexelCopyTextureInfo (Dawn 标准)，抛弃被删除的 WGPUImageCopyTexture
        ...
        // 💡 修复 3：统一使用 WGPUTexelCopyBufferLayout (Dawn 标准)，抛弃被删除的 WGPUTextureDataLayout
        ...
        wgpuQueueWriteTexture(bd->defaultQueue, &dst_view, tex->GetPixelsAt(upload_x, upload_y),
                              (uint32_t)(tex->Width * upload_h * tex->BytesPerPixel), &layout, &write_size);
...
bool ImGui_ImplWGPU_CreateDeviceObjects()
{
    ...
    attribute_desc[0] = { .nextInChain = nullptr, .format = WGPUVertexFormat_Float32x2, .offset = offsetof(ImDrawVert, pos), .shaderLocation = 0 };
    attribute_desc[1] = { .nextInChain = nullptr, .format = WGPUVertexFormat_Float32x2, .offset = offsetof(ImDrawVert, uv),  .shaderLocation = 1 };
    attribute_desc[2] = { .nextInChain = nullptr, .format = WGPUVertexFormat_Unorm8x4,  .offset = offsetof(ImDrawVert, col), .shaderLocation = 2 };
    ...
    // 💡 修复：直接配置 FragmentState
    WGPUFragmentState fragment_state = {};
    fragment_state.module = pixel_module;
    ...
    // 💡 修复：释放临时创建的 Module 句柄，因为 Pipeline 已持有它们的引用
    SafeRelease(vertex_module);
    SafeRelease(pixel_module);
...
bool ImGui_ImplWGPU_Init(ImGui_ImplWGPU_InitInfo* init_info)
{
    ...
    auto* bd = IM_NEW(ImGui_ImplWGPU_Data)();
...
{
    // 适配头文件中的 SuccessSuboptimal, Timeout 和 Outdated
    return (status == WGPUSurfaceGetCurrentTextureStatus_SuccessSuboptimal ||
            status == WGPUSurfaceGetCurrentTextureStatus_Timeout ||
            status == WGPUSurfaceGetCurrentTextureStatus_Outdated);
}

//-------------------------------------------------------------------------
// 辅助字符串转换函数 (严格匹配 webgpu.h)
//-------------------------------------------------------------------------
...
// 💡 记忆点：新标准中废弃了 WGPULogLevel，改为 WGPULoggingType
#ifndef __EMSCRIPTEN__
const char* ImGui_ImplWGPU_GetLoggingTypeName(WGPULoggingType type)
{
    switch (type)
    {
    case WGPULoggingType_Verbose: return "Verbose";
    case WGPULoggingType_Info:    return "Info";
    case WGPULoggingType_Warning: return "Warning";
    case WGPULoggingType_Error:   return "Error";
    default:                      return "Unknown";
    }
}
#endif
