...
namespace StuCanvas
{
    namespace utils
    {
        ...
        private:
            ...
            // =================================================================
            template <typename T>
            static void default_deleter(void* ptr) noexcept {
        ...
        public:
            ...
            [[nodiscard]] bool has() const noexcept {
                ...
                const uint32_t id = TypeRegistry::get_id<T>();
                if (id >= m_ptr->capacity) return false;

                auto* pairs = reinterpret_cast<IDPointer*>(m_ptr + 1);
                return pairs[id].pointer != nullptr && pairs[id].deleter == &default_deleter<T>;
            }
            ...
            [[nodiscard]] inline T* get() const noexcept {
                ...
                const uint32_t id = TypeRegistry::get_id<T>();
                if (id >= m_ptr->capacity) return nullptr;

                auto* pairs = reinterpret_cast<IDPointer*>(m_ptr + 1);
                if (pairs[id].deleter != &default_deleter<T>) return nullptr;
                return static_cast<T*>(pairs[id].pointer);
            }
