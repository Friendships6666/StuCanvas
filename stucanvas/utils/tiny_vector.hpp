...
namespace StuCanvas::utils
{
    ...
    private:
        ...
        // 🚀 终极对齐：锁定为 64 字节（CPU L1 缓存行大小），保证分配绝对 64 字节对齐，解锁最高速 AVX 矢量加载
        ...
        // 🚀 析离冷路径（Outlined Cold Path）：显式禁止内联，给主循环松绑
    ...
    public:
        ...
        // 🚀 极简内联热路径（Inlined Hot Path）：仅 3 行代码，最契合寄存器分配
        ...
        iterator erase(const_iterator first, const_iterator last)
        {
            ...
            if (f >= begin() && l <= end_it && f <= l)
            {
                ...
                std::move(l, end_it, f);
                for (iterator it = end_it - num_erased; it != end_it; ++it) { it->~T(); }
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // 2. C++23 针对指针类型的 Tagged Pointer 特化实现
    // ─────────────────────────────────────────────────────────────────────────
    template <typename T>
