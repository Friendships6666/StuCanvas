...
def create_combined_output(
    output_filename="project_summary.txt", items_to_scan=None, exclude_dirs=None
):
    """
    读取指定的目录或单个文件，
    ...
    if items_to_scan is None:
        items_to_scan = ["../stucanvas/objects/", "../stucanvas/utils/"]

    # 💡 新增：需要排除的目录列表
    if exclude_dirs is None:
        exclude_dirs = [
            "../stucanvas/assets",
            "../stucanvas/rust_bridge/",
            "../stucanvas/rust_bridge/typst_c/Cargo.lock",
            "../stucanvas/rust_bridge/typst_c/Cargo.toml",
        ]
        ...
        with open(output_filename, "w", encoding="utf-8") as output_file:
            for item in items_to_scan:
                ...
                if os.path.isfile(full_path):
                    process_single_file(
                        full_path, current_directory, output_file, all_file_paths
                    )
                ...
                # 💡 情况 2：如果是目录
                    ...
                    for root, dirs, files in os.walk(full_path):
                        ...
                        dirs[:] = [
                            d
                            for d in dirs
                            if os.path.normpath(os.path.join(root, d))
                            not in exclude_abs_paths
                        ]
