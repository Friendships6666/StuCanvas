...
int main() {
    std::cout << "================================================================================" << std::endl;
    std::cout << "      [对象装入 vector] 展开 (Expanded) vs 不展开 (Unexpanded) 性能对比" << std::endl;
    std::cout << "      测试内容：只读取前 8 个内联 double (hot_data)" << std::endl;
    std::cout << "================================================================================" << std::endl;
    std::cout << "正在初始化连续对象容器中...\n" << std::endl;

    // 🚀 A. 直接分配对象容器：堆分配器一次性划定 100,000 * 96 字节 = 9.6 MB 的纯净、无污染连续空间！
    ...
    // 🚀 B. 直接分配对象容器：一次性划定 100,000 * 8200 字节 = 820 MB 的超大连续空间
    ...
    std::cout << "不展开 (Unexpanded) 结构体大小: " << sizeof(SObjectUnexpanded) << " 字节" << std::endl;
    std::cout << "展开 (Expanded) 结构体大小: " << sizeof(SObjectExpanded) << " 字节\n" << std::endl;

    std::cout << std::left
              << std::setw(15) << "测试读取目标"
              << std::setw(28) << "不展开 (96B 实体) ms"
              << std::setw(28) << "展开 (8200B 实体) ms"
    ...
    for (size_t idx = 0; idx < 8; ++idx) {
        ...
        std::cout << std::left
                  << std::setw(15) << ("hot_data[" + std::to_string(idx) + "]")
                  << std::setw(28) << std::fixed << std::setprecision(4) << t_unexp
                  << std::setw(28) << std::fixed << std::setprecision(4) << t_exp
