// --- 文件路径: include/functions/functions.h ---
...
// 在每个函数定义前都加上 inline
inline FORCE_INLINE double safe_exp_scalar(double x) {
    if (x >= 1) return 1e270;
    if (x <= -100) return 1e-270;
    return std::exp(x);
}

inline FORCE_INLINE batch_type safe_exp_batch(const batch_type& x) {
...
inline FORCE_INLINE double check_ln_scalar(double x) {
    if (x <= 0.0) return std::numeric_limits<double>::quiet_NaN();
    return std::log(x);
}

inline FORCE_INLINE batch_type check_ln_batch(const batch_type& x) {
...
inline FORCE_INLINE double safe_ln_scalar(double x) {
    if (x > 0.0) return std::log(x);
    return -1e270;
}

inline FORCE_INLINE batch_type safe_ln_batch(const batch_type& x) {
    auto is_positive_mask = x > batch_type(0.0);
    batch_type log_result = xs::log(x);
    return xs::select(is_positive_mask, log_result, batch_type(-1e270));
}

inline FORCE_INLINE const xs::batch<double>& get_index_vec() {
